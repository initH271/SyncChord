    ...
    return (
        <div className="flex flex-col h-full">
            ...
            <div className="flex flex-col px-2 mt-3 gap-y-2  items-start justify-start">
                <WorkspaceSidebarItem label={"帖子"} id={"threads"} icon={MessageSquareText} variant={"active"}/>
                <WorkspaceSidebarItem label={"发送/草稿"} id={"drafts"} icon={SendHorizontal} variant={"default"}/>
            </div>
            <WorkspaceSidebarSection label={"所有频道"} hint={"新建频道"} onNew={() => {}}>
                {
                    channels?.map((ch) => (
                        <>
            ...
            <WorkspaceSidebarSection label={"所有成员"} hint={"添加成员"} onNew={() => {}}>
                {
                    members?.map((member) => (
